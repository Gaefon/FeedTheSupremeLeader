...
void Model::loadFile(string file_name)
{
	...
	if (fd.is_open())
	{
		while (getline(fd, line))
		{
			...
			{
				vector<string> coords = splitStr(data);
				tmp_uvs.push_back(glm::vec2(stof(coords.at(0)), stof(coords.at(1))));
			}
			else if (type == "f")
			{
				vector<string> coords = splitStr(data);
				for (vector<string>::iterator it = coords.begin(); it != coords.end(); it++)
				{
					...
					if (tmp_uvs.size() > 0)
						addVertice(tmp_vert.at(stoi(vertex.at(0)) - 1), glm::vec3(1.0f, 1.0f, 1.0f), tmp_normals.at(stoi(vertex.at(2)) - 1), tmp_uvs.at(stoi(vertex.at(1)) - 1));
					else
						addVertice(tmp_vert.at(stoi(vertex.at(0)) - 1), glm::vec3(1.0f, 1.0f, 1.0f), tmp_normals.at(stoi(vertex.at(2)) - 1));
				}
			}
		}
		fd.close();
